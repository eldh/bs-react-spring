%raw(`require('./Text.css')`)

let transX = x => j`translate3d(0,$(x)px,0)`
let items = list{"Lorem", "ipsum", "dolor", "sit"}
...
let make = () => {
  ...
  let (trail, setTrail) = Spring.useTrail(
      ...
      {React.array(
        Belt_Array.mapWithIndex(trail, (index, values) =>
          <Spring.Div
            ...
            <Spring.Div style={ReactDOM.Style.make(~height=values["height"], ())}>
