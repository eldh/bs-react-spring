...
type springData = {"x": float, "sc": int, "display": string}
module Springs = Spring.MakeSprings({
  type t = springData
...
let make = () => {
  ...
  let (props, set) = Springs.use(~number, i =>
    {"x": i->float_of_int *. windowWidthF, "sc": 1, "display": "block"}
  )
  let spreadProps = Gesture.useGesture(. prps => {
    let down = prps.down
    ...
    let sc = down ? (1. -. distance /. windowWidthF /. 2.)->int_of_float : 1
    if down && distance > windowWidthF /. 2. {
      cancel(. {
        let newVal = clamp(
          React.Ref.current(index) + (xDir->int_of_float > 0 ? -1 : 1),
          0,
          number - 1,
        )
    ...
    set(i => {
      let x = (i - React.Ref.current(index)) * windowWidth + (down ? xDelta->int_of_float : 0)
  ...
  <div className="pager">
    ...
    ->Belt.Array.mapWithIndex((i, p) => {
      ...
      <Spring.Div key={i->string_of_int} spreadProps style={ReactDOM.Style.make()}>
        <Spring.Div
          style={ReactDOM.Style.make(
            ~transform=p["sc"]->Springs.interpolate(s => j`scale($(s))`),
            ~backgroundImage=j`url($(url))`,
            (),
          )}
        />
      </Spring.Div>
